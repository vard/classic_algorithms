...
template <typename K, typename V> TreeNode<K, V>* BinTreeST<K, V>::min(TreeNode<K, V>* node){
    if(!node) return NULL;
    TreeNode<K, V>* largestValueNode = node;
    while(largestValueNode->left)
        largestValueNode = largestValueNode->left;
...
template <typename K, typename V>TreeNode<K, V>* BinTreeST<K, V>::remove(TreeNode<K, V>* node, const K& key) {
    ...
    if (key == node->key){
        ...
        TreeNode<K, V>* tmpNode = node;
        node = new TreeNode<K, V>(*min(node->right));
